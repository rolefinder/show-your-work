# recruit-me ΓÇö security posture
...
ΓÇ£Open-sourcing this gives attackers a surface map to hijack my site.ΓÇ¥
...
|-------|----------------|---------|
| Template app, Fit module, ingest skills, demo corpus | `hhalperin/recruit-me` | Private ΓåÆ public |
...
| Personal Terraform / zone IDs / production hostnames | harrison-site `infra/` | **Private** ΓÇö not copied into recruit-me docs |

Attackers who clone recruit-me get a **generic** free-tier template and a
**fake** persona corpus. They do **not** get HarrisonΓÇÖs content, bindings,
or account map.
...
anyone who loads harrisonhalperin.com. ΓÇ£Surface mapΓÇ¥ of the *production*
...
more than it helps attackers ΓÇö *if* production stays separated and secrets
stay out.
...
- Run **Publish Guardian** (or equivalent checklist) before first public
  push ΓÇö same spirit as harrison-siteΓÇÖs publication-safety ADR 002.
...
- Docs use `example.com` / `YOUR_ACCOUNT` ΓÇö never production IDs.
...
Fit logs to recruit-me ΓÇ£for completeness.ΓÇ¥

When Fit is live on the personal site, edge WAF / Bot Fight Mode /
application quota remain the production shields ΓÇö independent of whether
recruit-me is public.

---

## 5. ΓÇ£Surface mapΓÇ¥ mitigation checklist
...
- **Boundary Discipline.** Template boundary Γëá production boundary.  
- **Laziness.** Reuse existing OSS security tooling; donΓÇÖt invent a custom
  WAF product.  
- **Prove It Works.** Private ΓåÆ scanners green ΓåÆ public, not the reverse.
