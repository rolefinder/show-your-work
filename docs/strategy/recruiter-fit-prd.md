...
Paste or drop a job description → get a structured fit brief that maps each
...
2. Input: paste textarea **or** file drop (see §8).
3. Submit → progress → structured result panel (not a chat thread).
4. Result sections: role read-back, per-requirement rows, strongest matches,
   gaps, caveats.
5. Each `aligned` / `partial` row shows ≥1 link to `/work/<slug>` or
   `/writing/<slug>`.
...
5. Emit the output contract in §10. Refuse overall “perfect fit” sales copy.
...
| Size cap | 1 MB upload; ≤ ~12k characters of extracted text used (truncate with notice) |
...
- Output must cite evidence; empty retrieval → cannot be `aligned`.
