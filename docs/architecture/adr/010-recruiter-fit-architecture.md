# ADR 010: Recruiter Fit ΓÇö same-origin RAG, not training
...
LLM on site text (nanoGPT-style or a hosted ΓÇ£nano-gptΓÇ¥ API). The live site
is a static SPA on Cloudflare Pages with a strict CSP
(`connect-src 'self'`, no third-party scripts). There are no Pages
Functions today. The publishable corpus is small (~6ΓÇô7k words of project
...
A JD-fit product is a **comparison** problem (requirements Γåö evidence),
...
1. **Architecture = retrieval-augmented generation behind a same-origin
   ...
   knowledge path.
2. **Pipeline:** extract structured requirements from the JD ΓåÆ retrieve
   per requirement from an evidence index built from `visible` projects,
   posts, About, and approved summary fields ΓåÆ classify each requirement
   with citations ΓåÆ return the PRD output contract. Claims without
...
4. **Optional later:** Worker-side proxy to an external model API if
   Workers AI quality is insufficient ΓÇö still same-origin from the
...
- Shipping Fit requires introducing Pages Functions (and AI/Vectorize
  ...
  `docs/ops/bot-and-cost-protection.md` (Functions were previously ΓÇ£noneΓÇ¥).
...
- **Fine-tune a small instruct model on Q&A pairs as the sole knowledge
  ...
  facts and hard to audit; may be revisited later for tone or ΓÇ£say
  missingΓÇ¥ behavior only, still with RAG.
- **Browser-only WASM inference.** Rejected. Conflicts with CSP posture
  (ADR 007ΓÇÖs self-host rule is the precedent) and downloads large weights
  to visitors.
- **Third-party chat widget / browser `connect-src` to a model host.**
  Rejected. Breaks the siteΓÇÖs no-third-party CSP story and complicates
  secrets.
- **Static skill filter with no LLM.** Deferred as a degraded mode or
  abuse fallback; insufficient alone for detailed ΓÇ£whyΓÇ¥ prose recruiters
  want.
