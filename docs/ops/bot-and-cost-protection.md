...
> Cloudflare nameservers, nothing here can be applied — Terraform or dashboard. Treat all
...
| Workers AI / Vectorize (planned Recruiter Fit) | **None today; planned** | [`docs/strategy/recruiter-fit-prd.md`](../strategy/recruiter-fit-prd.md) + ADR 010–012 introduce same-origin `/api/fit`. Neuron/Vectorize spend becomes the metered risk; dual query caps (2/day **and** 10/week) and Request-more cooldown are mandatory before public launch. Update this table when Functions ship. |
...
and guardrails for the day a Function or paid product is added — not an enterprise WAF.

## 2. Recommended mitigations

### Baseline (all free — do these)

| Mitigation | Mechanism | IaC? |
|---|---|---|
| Bot Fight Mode | Challenges definitely-automated traffic zone-wide. Free-plan toggle. | Yes — `cloudflare_bot_management.fight_mode` in `security.tf` (needs token scope *Zone → Bot Management → Edit*; otherwise dashboard toggle). |
| Rate-limiting rule (1 free) | Blocks an IP exceeding 300 req/10s, excluding verified bots (Googlebot etc.). Free plan fixes period and timeout at 10s and action at block. | Yes — `cloudflare_ruleset.zone_rate_limit`. |
| WAF custom rule: block scanner probes | Blocks `/wp-login.php`, `/xmlrpc.php`, `/.env`, `/.git/…` and similar paths that only vuln scanners request. Uses 1 of 5 free custom rules. | Yes — `cloudflare_ruleset.zone_waf_custom`. |
...
| Billing / usage notifications | Email alert if any paid product ever starts accruing usage. | No — dashboard only (checklist). |

### Optional hardening (still free, off by default)

- **Block AI crawlers** — `ai_bots_protection = "block"` on the bot-management resource
...
- **Zone Cache Rule for long-lived assets** — commented in `security.tf`. Pages already
...
- **JavaScript Detections (`enable_js`)** — improves bot classification but injects a
...
### Paid tiers — explicitly not recommended
...
- **Full managed WAF rulesets / OWASP** (Pro+): there is no application behind this site
  to exploit — it's static files. The free managed ruleset is sufficient.
- **Bot Management** (Enterprise): no.

## 3. Turnstile: N/A (today)

Checked `app.jsx` and `index.html` (production surface) — there are **no forms** and no
...
- `cloudflare_bot_management.main` — Bot Fight Mode on.
- `cloudflare_ruleset.zone_rate_limit` — the single free rate-limiting rule.
- `cloudflare_ruleset.zone_waf_custom` — scanner-probe block rule.
...
set: **Zone → Zone WAF → Edit** (rulesets) and **Zone → Bot Management → Edit**.
...
1. **Re-scope the API token** — add *Zone → Zone WAF → Edit* and *Zone → Bot Management →
   Edit* so Terraform can own `security.tf`. Alternatively skip and do steps 2–3 by hand.
2. **Bot Fight Mode** — Security → Bots → toggle *Bot Fight Mode* on (only if not
   applying `security.tf` via Terraform).
3. **Verify rate-limit + WAF rules** after `terraform apply` — Security → WAF →
   Rate limiting rules / Custom rules should each show one rule.
4. **Billing notifications** — Manage Account → Notifications → add *Usage Based Billing*
...
5. **(Optional) Security Events review** — after a week, check Security → Events to
