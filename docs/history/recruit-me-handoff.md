...
**Repo:** private now → public later · **License:** Apache-2.0 · **Version:** 0.2.0
...
2. **Get the code onto `main`** (pick one):
   - **Preferred if this / a warm agent VM still has the tree:**
     ...
     ```
   - **If starting fresh:** unpack the source artifact (§9), then:
     ```bash
     cd recruit-me
...
3. **Verify**
   ...
   npm ci && npm run test && npm run preview   # :4173 — open /fit
   ```
4. **Then continue** from §8. Do **not** re-scaffold from scratch.
...
1. **Do not train** a tiny GPT on ~6–7k words of site prose as the product.
   Corpus is enough for RAG, not trustworthy from-scratch training.
   Hallucinated bio facts are worse than no bot.
2. **Optimal use case:** Recruiter pastes/drops a **job description** →
...
3. **Umbrella OSS product `recruit-me`:** open-source the website **template**
   ...
   `blog/`. Dogfood stays on private **harrison-site**; copy the harness —
...
ingest helpers, and a recruiter **Fit** surface (JD → cite-or-missing evidence
brief).
...
| License | **Apache-2.0** (true OSI — commercial use allowed) |
...
| LinkedIn | Official **data-export ZIP → parser** only. No scrapers / Voyager / cookie bots |
| Fit UX | Still open: single-shot panel (current) vs thin multi-turn shell later |
| Demo data | Fictional **Avery Quill** only — never Harrison’s real bio/projects |

Strategy docs live on **harrison-site** draft PRs (may be unmerged):

| Doc | Where |
|-----|--------|
| Fit PRD + ADRs 010–013 | PR #86 `cursor/recruiter-fit-prd-adrs-e55a` |
| Umbrella PRD + OSS plan | PR #88–#89 |
...
- Build: `tsc --noEmit` + **esbuild** IIFE → `dist/app.js`
...
### Fit (working, deterministic — no LLM yet)
- UI: `src/fit/FitPage.tsx` — paste or drop `.txt` / `.md` (1 MB / 12k char caps)
...
- `scripts/emit-evidence.py` → `dist/evidence.json`
- Pages Function: `functions/api/fit.ts` (`POST /api/fit`), optional KV `FIT_QUOTA` (2/day stub)
- Worker bundle: `npm run build:fit-worker` → `functions/_lib/fit-engine.js` (gitignored)
- Smoke: `scripts/fit-smoke.ts` — CI/CD JD must cite Harbor Gate; Kubernetes must **not** be aligned on demo corpus

### Ingest (draft generators — human review before copy into `content/`)
- `packages/ingest/from-resume-text.py`
- `packages/ingest/from-github.py` (public GitHub API)
- `packages/ingest/README.md` — LinkedIn ZIP path documented, parser not built

### Ops / security baseline
- `skills/infra-pages/SKILL.md` — Cloudflare Pages free-tier standup
...
```
<docs handoff commits>   # HANDOFF.md — see git log (tip ~cb2cce6)
...
cc9b089 feat: v0 scaffold — YAML content, emit, CSP static build
```
...
pip install --user pyyaml
npm ci
npm run build          # emit → typecheck → esbuild → evidence.json → fit-worker
...
between `/* BEGIN … */` markers without understanding emit.
...
4. **Demo corpus ≠ production.** Never copy harrison-site real YAML into this repo.
...
## 7. Security / “surface map” concern (Harrison)
...
1. **Land code on GitHub `main`** (§0) and green CI on the remote.
2. **Workers AI + Vectorize** behind `/api/fit` — same response contract; keep
   deterministic path as fallback / local preview.
3. **Quota** — implement ADR 011 dual caps (2/day **and** 10/week) + Request-more
   (larger model, 7-day cooldown, +2 grant) when AI path exists.
4. **LinkedIn export ZIP parser** — adapt OSS ZIP→structured patterns; human
   confirm before write. No scrapers.
5. **PDF JD** on Worker only (`.docx` deferred).
6. **2–3 themes** + design skill (copyright-aware “inspiration,” not paste
   proprietary CSS).
7. **Publish Guardian / public flip** checklist, then Work card on harrison-site:
   `content/projects/recruit-me.yaml` with `note.href` → GitHub.
...
| Artifact | SHA-256 |
|----------|---------|
| `/home/ubuntu/recruit-me` | Working tree if warm — **preferred** (HEAD `cb2cce6`) |
...
| PR | Branch | Topic |
|----|--------|--------|
| #86 | `cursor/recruiter-fit-prd-adrs-e55a` | Fit PRD + ADRs 010–013 |
| #87 | `cursor/content-emit-modular-posts-e55a` | Modular posts + shared emit helpers |
| #88–#89 | recruit-me vision branches | Umbrella PRD + OSS packaging |
...
mirroring ADR 006 projects — see PR #87 / ADR 013.
...
2. Starter theme set (which 2–3 looks)?
...
6. Finish clarifying GitHub App / collaborator access for agents on `recruit-me`
   (prior message was cut off at “You gave access to…”).
...
- Do not put Harrison’s real portfolio YAML in this repo
...
- Do not `git push` to `harrison-site` `main` or edit protected CI paths there
  without following that repo’s agent guide
- Do not force-push `recruit-me` `main` if Harrison already has commits you
  haven’t seen — coordinate first
...
speculative refactors. When in doubt, re-read §3 locked decisions and §6
architecture notes.
